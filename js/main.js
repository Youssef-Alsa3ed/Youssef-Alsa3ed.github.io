/* ── Particle + geometric shapes canvas ── */
...
function initShapes() {
  shapes = [
    ...
    { type:'pentprism', x:W*0.08, y:H*0.25, size:25, rot:0.5, speed:0.002 },
    // Top-right area
    { type  :'sq',  x:W*0.95, y:H*0.28, size:25, rot:0.5, speed:0.002 },
    { type:'prism', x:W*0.92, y:H*0.32, size:30, rot:1.5, speed:0.009 },
    { type:'tri', x:W*0.85, y:H*0.08, size:28, rot:0, speed:0.006 },
    // Hexagonal pyramids (new, top area)
    { type:'hexpyramid', x:W*0.1, y:H*0.15, size:25, rot:0, speed:0.005 },
    { type:'hexpyramid', x:W*0.9, y:H*0.15, size:22, rot:1, speed:0.0055 },
...
function tick() {
  ...
  for(const s of shapes) {
    ...
    else if(s.type==='tri') { drawTri(s.x, s.y, s.size, s.rot); ctx.stroke(); }
    else if (s.type==='cube') { drawCube(s.x, s.y, s.size, s.rot); }
    else if (s.type==='prism') { drawPrism(s.x, s.y, s.size, s.rot); }
    else if (s.type==='hexpyramid') { drawHexagonalPyramid(s.x, s.y, s.size, s.rot); }
    else if (s.type==='pentprism') { drawPentagonalPrism(s.x, s.y, s.size, s.rot); }
    else { drawSq(s.x, s.y, s.size, s.rot); ctx.stroke(); }

    // second ring (golden)
    ctx.strokeStyle = GOLD + '0.4)';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    if(s.type==='hex') { drawHex(s.x, s.y, s.size*1.6, -s.rot*0.5); ctx.stroke(); }
    else if(s.type==='tri') { drawTri(s.x, s.y, s.size*1.5, -s.rot*0.6); ctx.stroke(); }
    else if (s.type==='cube') { drawCube(s.x, s.y, s.size*1.6, s.rot); }
    else if (s.type==='prism') { drawPrism(s.x, s.y, s.size*1.6, s.rot * 0.5); }
    else if (s.type==='pentprism') { drawPentagonalPrism(s.x, s.y, s.size*1.6, s.rot); }
    else if (s.type==='hexpyramid') { drawHexagonalPyramid(s.x, s.y, s.size*1.6, -s.rot*0.5); }
    else if (s.type==='sq') { drawSq(s.x, s.y, s.size*1.8, -s.rot*0.7); ctx.stroke(); }
  }
...
window.addEventListener('resize', () => { resize(); initParticles(); initShapes(); });
resize();
initParticles();
initShapes();
tick();
